...
build_desktop()
{
	local XDESKTOP=`cat $MOUNT_POINT/mnt/software/xdesktop.txt`
	
	for DIRECTORY in $XDESKTOP
	do
		if [ -d $MOUNT_POINT/mnt/software/$DIRECTORY ]
		then
			build_packages $DIRECTORY root &&
			activate_packages $DIRECTORY runtimes /usr/X11R6/software &&
